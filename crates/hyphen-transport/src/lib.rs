...
impl SignedEnvelope {
    pub fn sign(msg_type: u32, payload: Vec<u8>, sk: &hyphen_crypto::SecretKey) -> Self {
    ...
    pub fn verify(&self) -> bool {
        ...
        let pk_bytes: [u8; 32] = match self.sender_pubkey.as_slice().try_into() {
            Ok(b) => b,
            Err(_) => return false,
        };
        let pk = hyphen_crypto::PublicKey(pk_bytes);
...
#[derive(Clone, prost::Message)]
pub struct TemplateInvalidated {
    #[prost(bytes = "vec", tag = "1")]
    pub old_template_id: Vec<u8>,
    #[prost(uint32, tag = "2")]
    pub reason: u32,
}

pub const INVALIDATION_NEW_BLOCK: u32 = 1;
