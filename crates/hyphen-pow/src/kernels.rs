...
fn kernel_sparse_step(page: &[u8], state: &[u8; 64], ep: &EpochKernelParams) -> [u8; 64] {
    ...
    for step in 0..48u64 {
        ...
        let slot = step as usize % 8;
        acc[slot] = acc[slot].wrapping_add(val.wrapping_mul(acc[(slot + 1) % 8] | 1));
        acc[(slot + 4) % 8] ^= val.rotate_left((acc[slot] & 31) as u32).wrapping_mul(mix_c);
    }
    to_output(&acc)
}
...
fn kernel_micro_sort(page: &[u8], state: &[u8; 64], ep: &EpochKernelParams) -> [u8; 64] {
    ...
    let mut arr = [0u64; 32];
    for i in 0..32 {
        arr[i] = read_u64_le(page, i.wrapping_mul(stride) % page.len()) ^ state_u64(state, i % 8);
    }
    for i in 1..32 {
        let key = arr[i];
        let mut j = i;
    ...
    for i in 0..32 {
        acc[i % 8] =
            acc[i % 8].wrapping_add(arr[i].wrapping_mul((i as u64 + 1).wrapping_mul(mix_c)));
    }
    to_output(&acc)
}
...
fn kernel_hash_mix(page: &[u8], state: &[u8; 64], ep: &EpochKernelParams) -> [u8; 64] {
    ...
    for round in 0..16u32 {
        let page_off =
            (round as usize).wrapping_mul(stride) % (page.len().saturating_sub(16).max(1));
...
fn kernel_branch_maze(page: &[u8], state: &[u8; 64], ep: &EpochKernelParams) -> [u8; 64] {
    ...
    for step in 0..64u64 {
        ...
        match val & 0x07 {
            ...
            _ => {
                acc[slot] = acc[slot].wrapping_add(val).wrapping_mul(mix_c);
...
fn kernel_scatter_gather(page: &[u8], state: &[u8; 64], ep: &EpochKernelParams) -> [u8; 64] {
    ...
    for step in 0..80u64 {
        ...
        let scratch_val =
            u64::from_le_bytes(scratch[scratch_idx..scratch_idx + 8].try_into().unwrap());
