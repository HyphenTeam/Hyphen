...
全强度 VRE 参数（如 `vre_age_band_width = 2048`）需要数千个区块后才能构造交易。Hyphen 通过**自适应 VRE** 解决此问题：共识层和钱包联合按当前链高度等比例缩放参数，使得所有四条 VRE 规则在达到**激活高度**后始终可满足。

- **VRE 激活高度：** 主网 = 128，测试网 = 32。激活高度之前仅存在 coinbase（挖矿奖励）交易；不接受用户转账。
- **有效带宽：** $w_{\text{eff}} = \min\bigl(w,\; \lfloor h / B_{\min} \rfloor\bigr)$ — 缩小带宽使所需的不同年龄带数量始终适合可用高度范围。
- **有效最小环跨度：** $S_{\text{eff}} = \min(S_{\min},\; h - 1)$ — 将跨度要求限制在链实际可提供的范围内。
- **渐进式索引跨度（logistic 斜坡）：** $\tau_{\text{eff}} = \min\!\Bigl(\tau \cdot \frac{n^2}{n^2 + k^2},\; \frac{(N-1) \cdot 10000}{N}\Bigr)$，其中 $k = \texttt{ring\_size} \times 64$。与简单上限不同，此 logistic sigmoid 提供平滑的 0 → 目标增长：在 $n = k$ 个输出时达到 50%，$n = 2k$ 时达到 80%，随输出集增长收敛至全强度执行。

随着链的增长，所有有效参数收敛至论文指定的全强度值。主网高度 6144（$= 2048 \times 3$）时，$w_{\text{eff}}$ 达到 2048，全强度 VRE 生效。

#### 安全加固

Hyphen 在所有交易接受路径上实施纵深防御：

- **P2P gossip 验证。** 通过 gossipsub 接收的每笔交易在进入内存池前必须通过完整共识验证（CLSAG + TERA + MD-VRE + 余额 + 范围证明 + 无效化检查）。恶意对等节点无法向任何诚实节点的池中注入无效交易。
- **验证门控内存池插入。** 内存池 API 要求 `Validated` 证明令牌，确保没有任何代码路径可以插入未验证的交易。该令牌还携带 **VRE 质量分数** (0–10,000) — 见下文。
- **创世配置不可变性。** 节点首次启动时将共识关键参数的 blake3 哈希持久化。每次后续启动时比较存储的哈希与当前配置；不匹配则中止并显示明确错误，防止意外或恶意的规则变更。
- **VRE 质量评分。** 每笔已验证交易获得基于四个等权因子的综合质量分数：高度跨度超额、高度唯一性、年龄带多样性和索引扩展度。内存池将此分数作为手续费密度之后的二级优先级（通过 `Priority` 排序中的 `neg_vre_quality`），使具有更优环构造的交易被优先纳入区块。
- **诱饵分布审计。** 钱包将输出索引空间划分为 10 个等宽区间，验证节点返回的诱饵中不超过一半落入同一区间。若节点返回聚集输出以去匿名化发送者，将被检测到并拒绝交易。
- **自适应 VRE 标志。** 钱包向 Flutter UI 报告 `vre_used_adaptive: bool` 字段。当使用了自适应参数时，用户会看到信息横幅，说明随链成熟环熵将自动提升。
