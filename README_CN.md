...
Hyphen 将内存硬多内核 PoW、完整隐私原语（CLSAG 环签名、Bulletproofs 范围证明、隐身地址）、后量子混合签名、NTP 同步毫秒级时间戳、GHOST 风格的叔块机制、MDAD-SPR 多相位难度调整和抗 51% 难度阻尼整合到一个生产级区块链中。
...
| `hyphen-mempool` | 按手续费优先的交易池，含密钥映像冲突检测 |
...
| `hyphen-rpc` | 长度前缀 protobuf RPC — 链信息查询、区块查询、交易提交、输出索引查询（GET_RANDOM_OUTPUTS 用于诱饵选择） |
...
**ASIC 抗性定理。** 令 $\mathcal{K} = \{K_0, \ldots, K_{11}\}$ 为内核集。对于给定 nonce，内核调用序列 $\sigma = (\sigma_1, \ldots, \sigma_{1024})$（其中 $\sigma_i \in \mathcal{K}$）取决于运行时状态。针对任意严格子集 $\mathcal{K}' \subset \mathcal{K}$ 优化的 ASIC 至少产生如下减速因子：

$$S \geq \frac{|\mathcal{K}|}{|\mathcal{K}'|} \cdot \frac{T_{\text{emulate}}}{T_{\text{native}}}$$

其中 $T_{\text{emulate}} / T_{\text{native}}$ 是缺失内核的模拟执行与原生执行成本之比。由于内核涵盖整数除法、AES 流水线、分支预测、浮点运算和缓存层次结构，没有单一硬件架构能同时加速所有内核。
...
3. **CLSAG 环签名** — Ristretto255 上的紧凑可链接环签名，含密钥映像和承诺密钥映像
...
**Hyphen 的 VRE** 通过两条共识强制规则解决此问题：

**规则 1：最小高度跨度。** 给定环成员区块高度 $\{h_1, \ldots, h_n\}$：

$$\max(h_i) - \min(h_i) \geq S_{\min}$$

其中 $S_{\min} = 100$（主网），$S_{\min} = 20$（测试网）。确保环成员跨越显著的时间范围，防止时间聚类攻击。

**规则 2：最小不同高度比例。** 设 $D = |\{h_1, \ldots, h_n\}|$ 为环中不同高度的数量：

$$D \geq \lceil 3n/4 \rceil$$

对于环大小 16，至少 12 个成员必须来自不同的区块高度。防止从热门区块大量重用诱饵。

**安全性提升。** 在 VRE 下，有效匿名集是可证明有下界的。没有 VRE 时，所有诱饵来自高度 $h$ 的交易有效匿名度 $\leq 1$（真实输入是唯一来自不同高度的成员，可被轻易识别）。有 VRE 时，最小有效匿名度为：
...
- **私密转账** — 完整的隐蔽交易流水线：通过 RPC 扫描区块链输出、从节点输出索引获取诱饵输出（GET_RANDOM_OUTPUTS）构建 CLSAG 环签名、Bulletproofs 范围证明、bincode 序列化并提交交易到内存池；贪心输入选择与自动找零输出生成
- **NFC 无接触** — 通过 NFC 触碰分享钱包地址，Android 支持完整 NFC，iOS 支持 NDEF/TAG 读取会话（已配置 entitlements）；使用 `nfc_manager` 3.5.0，已包含 Kotlin 2.2 兼容性处理
- **生物识别认证** — 通过 `local_auth` 2.3.0 支持指纹和 Face ID 解锁；助记词查看和交易确认受生物识别保护
